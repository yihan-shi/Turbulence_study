...
```{r}
# least square model
r2_ls <- lm(log(R_moment_2) ~ St + Re + Fr_factor, data = sub_train)
r2_ls_int <- lm(log(R_moment_2) ~ St + Re + Fr_factor + Re*Fr_factor, data = sub_train)
summary(r2_ls)
summary(r2_ls_int)
```
...
```{r}
#ridge regression
train.matrix <- model.matrix (log(R_moment_2) ~ St + Re + Fr_factor + Re*Fr_factor, sub_train)[,-1]
test.matrix <- model.matrix (log(R_moment_2) ~ St + Re + Fr_factor + Re*Fr_factor, sub_test)[,-1]
...
RSS <- rep(0,15)
for (i in 4:15) {
model.fit <- glm(log(R_moment_2) ~ bs(St, df = i) + Re + Fr_factor + Re*Fr_factor, data = sub_train)
...
```{r}
r2_spline <- glm(log(R_moment_2) ~ bs(St, df = 6) + Re + Fr_factor + Re*Fr_factor, data = sub_train)
attr(bs(sub_train$St, df = 6), "knots")
...
```{r}
r2_gam1 <- gam(log(R_moment_2) ~ s(St, 3) + Re + Fr_factor + Re*Fr_factor, data = sub_train)
r2_gam2 <- gam(log(R_moment_2) ~ s(St, 5) + Re + Fr_factor+ Re*Fr_factor, data = sub_train)
r2_gam3 <- gam(log(R_moment_2) ~ s(St, 7) + Re + Fr_factor+ Re*Fr_factor, data = sub_train)
r2_gam4 <- gam(log(R_moment_2) ~ s(St, 9) + Re + Fr_factor+ Re*Fr_factor, data = sub_train)
...
```{r}
r2_gam3_no_int <- gam(log(R_moment_2) ~ s(St, 7) + Re + Fr_factor, data = sub_train)
```
