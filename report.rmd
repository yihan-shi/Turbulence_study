...
adj.R <- c(mse_adjr2,
           ...
           max_r2_ns,
           max_r2_gam)
formula <- c("log(R_moment_1) ~ Fr + Re + St + Fr * Re",
...
fit.pred2 <- exp(predict(r2_gam, newdata=test))
```
...
for(i in 1:K){
    ...
    #use k-fold cv to evaluate models
    for (j in 3:13){
        spline = glm(log(R_moment_3) ~ ns(St_log, df = j)+Re+ns(Fr_logit,2)+Re*Fr_logit, data = trainData)
...
```{r predict r2}
r3_gam <- gam(log(R_moment_3) ~ ns(log(St), df = 3) + Re + ns(Fr_logit, df = 2) + Re:ns(Fr_logit,2), data = train)
fit.pred3 <- exp(predict(r2_gam, newdata=test))
```
...
fit.pred4 <- exp(predict(r4_gam, newdata=test))
```
...
for(i in 1:K){
    ...
    #use k-fold cv to evaluate models
    for (j in 3:13){
        spline = glm(log(C_moment_3) ~ ns(St_log, df = j)+Re+ns(Fr_logit,2)+Re*Fr_logit, data = trainData)
        fit.test = predict(spline, newdata=testData)
...
```{r}
spline <- glm(log(C_moment_3) ~ ns(St_log, df = 3)+Re+ns(Fr_logit,2)+Re*Fr_logit, data = trainData)
summary(spline)
```
