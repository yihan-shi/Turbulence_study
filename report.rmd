...
test$Fr_logit <-  asymptote / (1 + exp((midpoint - test$Fr) * scale))
train$Fr_logit
```
...
train$C_moment_3 <- (train$R_moment_3 - 3*train$R_moment_1*train$R_moment_2 + 2*(train$R_moment_1)^3)/((train$C_moment_2)^(3/2))
train$C_moment_4 <- (train$R_moment_4 - 4*train$R_moment_1*train$R_moment_3 + 6*(train$R_moment_1)^2*train$R_moment_2 -3*(train$R_moment_1)^4)/((train$C_moment_2)^2)
```
...
for(i in 1:K){
    ...
    fit.train <-  lm(log(R_moment_1) ~ St + Re + Fr_logit, data = trainData)
...
#find MSE
mse_linear_interaction <- mean(mse)
mse_adjr2_interaction <- mean(adj.r2)
```
...
Using cross-validation with 5 folds, we see the adjusted $R^2$ decreases as polynomial degree increases. The MSE indicates that linear regression is not sufficient. The minimum MSE is achieved with polynomial order 2 on St. We decided that the optimal polynomial order for St is 2 for better interpretation.
...
```{r}
r1_poly <- lm(log(R_moment_1) ~ poly(St,2) + Re + Fr_logit + Re*Fr_logit, data = train)
summary(r1_poly)
```
...
```{r predict r1}
fit.gam1 <- gam(log(R_moment_1) ~ s(St, df = 13) + Fr_logit + Re + Fr_logit * Re, data = train)
fit.pred1 <- exp(predict(fit.gam1, newdata=test))
```
...
models <- c("Linear regresion", 
            "Linear regresion with interactions",
            "Polynomial regression", 
            "Natural spline", 
            "Generalized additive model")
mse <- c(mse_linear,
         mse_linear_interaction, 
         min_mse_linear, 
         min_mse_ns, 
         min_mse_gam)
adj.R <- c(mse_adjr2,
           mse_adjr2_interaction,
           max_r2_linear,
           max_r2_ns,
           max_r2_gam)
formula <- c("log(R_moment_1) ~ Fr + Re + St",
             "log(R_moment_1) ~ Fr + Re + St + Fr * Re",
             "log(R_moment_1) ~ Fr + Re + poly(St, 2) + Fr * Re",
             "log(R_moment_1) ~ ns(St, df = 7) + Fr + Re + Fr * Re",
             "log(R_moment_1) ~ s(St, 13) + Re + Fr + Fr * Re"
             )
...
For Raw_moment_1 (mean), GAM has the best results since it presents the highest adjusted $R^2$ and the lowest MSE. The second best model is natural spline, with only slight increase in MSE. Since both are hard to interpret, we use it for prediction and polynomial regression for inference.
...
for(i in 1:K){
    ...
    #use k-fold cv to evaluate models
    fit.train = lm(log(R_moment_2) ~ St + Re + Fr_logit, data = trainData)
    fit.test = predict(fit.train, newdata=testData)
...
```{r}
mse_ls_interaction <- mean(mse)
mse_ls_interaction

adjR2_ls_interaction <- max(adj.r2)
adjR2_ls_interaction
```
...
```{r predict r2}
fit.gam2 <- gam(log(R_moment_2) ~ ns(St, df = 4) + Re + ns(Fr_logit, df = 2) + Re:ns(Fr_logit,2), data = train)
fit.pred2 <- exp(predict(fit.gam2, newdata=test))
```
...
adj.R <- c(adjR2_ls,
           ...
           adjR2_spline,
           adjR2_gam)
formula <- c("log(R_moment_2) ~ Fr + Re + St",
             "log(R_moment_2) ~ Fr + Re + St + Fr * Re",
...
RSS <- rep(0,13)
for (i in 3:13) {
  spline <- glm(log(R_moment_2) ~ ns(St, df = i) + Re + Fr_logit + Re*Fr_logit, data = train)
...
for(i in 1:K){
    ...
    #use k-fold cv to evaluate models
    for (j in 3:13){
        spline = glm(log(R_moment_3) ~ ns(St_log, df = j)+Re+Fr_logit+Re*Fr_logit, data = trainData)
...
```{r}
spline <- glm(log(R_moment_3) ~ ns(St_log, df = 3)+Re+ns(Fr_logit,2)+Re*Fr_logit, data = trainData)
summary(spline)
```
...
RSS <- rep(0,13)
for (i in 3:13) {
  gam <- glm(log(R_moment_2) ~ bs(St, df = i) + Re + Fr_logit + Re*Fr_logit, data = train)
  RSS[i] <- sum(gam$residuals^2)
}
plot(3:13, RSS[3:13], type="b", xlab="Degrees of freedom", ylab = "RSS")
title("RSS vs. Degrees of freedom")
```
...
folds <- cut(seq(1,nrow(df.shuffled)),breaks=K,labels=FALSE)
#create object to hold MSE's of models
gam_mse = matrix(data=NA,nrow=K,ncol=13)
gam_R2 <- matrix(data=NA,nrow=K,ncol=13)
#Perform K-fold cross validation
for(i in 1:K){
    ...
    #use k-fold cv to evaluate models
    for (j in 3:13){
        gam = gam(log(R_moment_3) ~ ns(St_log, df = j)+Re+ns(Fr_logit,2)+Re*ns(Fr_logit,2), data = trainData)
        fit.test = predict(gam, newdata=testData)
        gam_mse[i,j] = mean((exp(fit.test)-testData$R_moment_3)^2) 
        gam_R2[i,j] <- with(summary(gam), 1 - deviance/null.deviance)
    }
}
```
...
colMeans(gam_mse)
colMeans(gam_R2)
min(colMeans(gam_mse), na.rm=TRUE)
max(colMeans(gam_R2), na.rm=TRUE)
```

```{r}
gam <- gam(log(R_moment_3) ~ ns(St_log, df = 3)+Re+ns(Fr_logit,2)+Re*ns(Fr_logit,2), data = trainData)
summary(gam)
```
...
```{r}
mse_gam <- colMeans(gam_mse)[3]
mse_gam
R2_gam <- colMeans(gam_R2)[3]
R2_gam
```
...
formula <- c("log(R_moment_3) ~ log(St)+Re+logit(Fr)",
            "log(R_moment_3) ~ log(St)+Re+logit(Fr)+Re*logit(Fr)",
             ...
             "log(R_moment_3) ~ ns(log(St),df=3)+Re+logit(Fr)+Re*logit(Fr)", 
            "log(R_moment_3) ~ ns(log(St),df=3)+Re+ns(logit(Fr),2)+Re*ns(logit(Fr),df=2)"
             )
...
```{r predict r3}
fit.gam3 <- gam(log(R_moment_3) ~ ns(log(St), df = 3) + Re + ns(Fr_logit, df = 2) + Re:ns(Fr_logit,2), data = train)
fit.pred3 <- exp(predict(fit.gam3, newdata=test))
```
...
```{r predict r4}
fit.gam4 <- gam(log(R_moment_4) ~ St + Re + ns(Fr_logit,2) + Re*ns(Fr_logit,2) , data = train)
fit.pred4 <- exp(predict(fit.gam4, newdata=test))
```

```{r}
par(mfrow = c(1,3))
plot(fit.gam4, se = TRUE, col = "blue")
```
...
for(i in 1:K){
    ...
    #use k-fold cv to evaluate models
    fit.train = lm(log(C_moment_2) ~ St + Re + Fr_logit, data = trainData)
    fit.test = predict(fit.train, newdata=testData)
...
adj.R <- c(adjR2_ls,
           ...
           adjR2_spline,
           adjR2_gam)
formula <- c("log(C_moment_2) ~ Fr + Re + St",
             "log(C_moment_2) ~ Fr + Re + St + Fr * Re",
...
```{r}
plot(density(train$C_moment_3))
```

```{r}
plot(density(log(train$C_moment_3)))
```

```{r}
train$C_moment_3_log = log(train$C_moment_3)
var <- c("St_log", "Re", "Fr_logit", "R_moment_3_log")
cor <- pairs(train[,(names(train) %in% var)])
cor
```
...
for(i in 1:K){
    ...
    #use k-fold cv to evaluate models
    linear = lm(log(C_moment_3)~St_log+Re+Fr_logit, data = trainData)
    fit.test = predict(linear, newdata=testData)
    linear_mse[i] = mean((exp(fit.test)-testData$C_moment_3)^2) 
    linear_R2[i] <- summary(linear)$adj.r.squared
}
```
...
```{r}
mse_linear_C <- mean(linear_mse)
mse_linear_C
R2_linear_C <- max(linear_R2)
R2_linear_C
```
...
for(i in 1:K){
    ...
    #use k-fold cv to evaluate models
    interaction = lm(log(C_moment_3)~St_log+Re+Fr_logit+Re*Fr_logit, data = trainData)
    fit.test = predict(interaction, newdata=testData)
    interaction_mse[i] = mean((exp(fit.test)-testData$C_moment_3)^2) 
    interaction_R2[i] <- summary(interaction)$adj.r.squared
}
```
...
```{r}
mse_interaction_C <- mean(interaction_mse)
mse_interaction_C
R2_interaction_C <- max(interaction_R2)
R2_interaction_C
```
...
for(i in 1:K){
    ...
    #use k-fold cv to evaluate models
    for (j in 1:10){
        poly = lm(log(C_moment_3) ~ poly(St_log,j)+Re+poly(Fr_logit,2)+Re*Fr_logit, data = trainData)
        fit.test = predict(poly, newdata=testData)
        poly_mse[i,j] = mean((exp(fit.test)-testData$C_moment_3)^2) 
        poly_R2[i,j] <- summary(poly)$adj.r.squared
    }
}
```
...
```{r}
poly <- lm(log(C_moment_3) ~ poly(St_log,3)+Re+poly(Fr_logit,2)+Re*poly(Fr_logit,2), data = train)
```
...
```{r}
mse_poly_C <- colMeans(poly_mse)[3]
mse_poly_C
R2_poly_C <- colMeans(poly_R2)[3]
R2_poly_C
```
...
for(i in 1:K){
    ...
    #use k-fold cv to evaluate models
    for (j in 3:13){
        spline = glm(log(C_moment_3) ~ ns(St_log, df = j)+Re+Fr_logit+Re*Fr_logit, data = trainData)
        fit.test = predict(spline, newdata=testData)
        spline_mse[i,j] = mean((exp(fit.test)-testData$C_moment_3)^2) 
        spline_R2[i,j] <- with(summary(spline), 1 - deviance/null.deviance)
    }
}
```
...
```{r}
spline <- glm(log(C_moment_3) ~ ns(St_log, df = 3)+Re+Fr_logit+Re*Fr_logit, data = trainData)
summary(spline)
```
...
```{r}
mse_spline_C <- colMeans(spline_mse)[3]
mse_spline_C
R2_spline_C <- colMeans(spline_R2)[3]
R2_spline_C
```
...
for(i in 1:K){
    ...
    #use k-fold cv to evaluate models
    for (j in 3:13){
        gam = gam(log(C_moment_3) ~ ns(St_log, df = j)+Re+ns(Fr_logit,2)+Re*ns(Fr_logit,2), data = trainData)
        fit.test = predict(gam, newdata=testData)
        gam_mse[i,j] = mean((exp(fit.test)-testData$C_moment_3)^2) 
        gam_R2[i,j] <- with(summary(gam), 1 - deviance/null.deviance)
    }
}
```
...
```{r}
gam <- gam(log(C_moment_3) ~ ns(St_log, df = 3)+Re+ns(Fr_logit,2)+Re*ns(Fr_logit,2), data = trainData)
summary(gam)
```
...
```{r}
mse_gam_C <- colMeans(gam_mse)[3]
mse_gam_C
R2_gam_C <- colMeans(gam_R2)[3]
R2_gam_C
```
...
R2 <- c(R2_linear_C,
        ...
        R2_spline_C, 
        R2_gam_C)
formula <- c("log(C_moment_3) ~ log(St)+Re+logit(Fr)",
            "log(C_moment_3) ~ log(St)+Re+logit(Fr)+Re*logit(Fr)",
             "log(C_moment_3) ~ poly(log(St),3)+Re+poly(logit(Fr),2)+Re*logit(Fr)",
             "log(C_moment_3) ~ ns(log(St),df=3)+Re+logit(Fr)+Re*logit(Fr)", 
            "log(C_moment_3) ~ ns(log(St),df=3)+Re+ns(logit(Fr),2)+Re*ns(logit(Fr),2)"
             )
df <- data.frame(models, formula, mse, R2)
df %>% 
  kbl() %>% 
...
for(i in 1:K){
    ...
    #use k-fold cv to evaluate models
    fit.train = lm(log(C_moment_4) ~ St + Re + Fr_logit, data = trainData)
    fit.test = predict(fit.train, newdata=testData)
...
```{r}
c4_poly <- lm(log(C_moment_4) ~ poly(St,2) + Re + poly(Fr_logit,2) + Re*Fr_logit, data = train)
summary(c4_poly)
```
...
adj.R <- c(adjR2_ls,
           ...
           adjR2_spline,
           adjR2_gam)
formula <- c("log(C_moment_4) ~ Fr + Re + St",
             "log(C_moment_4) ~ Fr + Re + St + Fr * Re",
...
# Prediction
```{r}
test$raw_moment_1 <- fit.pred1
test$raw_moment_2 <- fit.pred2
test$raw_moment_3 <- fit.pred3
test$raw_moment_4 <- fit.pred4
test
write.csv(test, here::here("data-test.csv"))
```

```{r}
plot(density(test$raw_moment_1))
plot(density(test$raw_moment_2))
plot(density(test$raw_moment_3))
plot(density(test$raw_moment_4))
summary(test$raw_moment_1)
summary(test$raw_moment_2)
summary(test$raw_moment_3)
summary(test$raw_moment_4)
boxplot(test$raw_moment_1,test$raw_moment_2,test$raw_moment_3,test$raw_moment_4, horizontal = TRUE)
boxplot(test$raw_moment_2, horizontal = TRUE)
boxplot(test$raw_moment_3,test$raw_moment_4, horizontal = TRUE, outline=FALSE)
```

```{r}
boxplot(train$R_moment_4, horizontal = TRUE)
```

```{r}
# basic example
ggplot(boxplot, aes(x = price, y = cut, fill = cut)) +
  geom_density_ridges() +
  theme_ridges() + 
