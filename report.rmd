...
for(i in 1:K){
    ...
    #use k-fold cv to evaluate models
    for (j in 3:13){
        spline = glm(log(C_moment_3) ~ ns(St_log, df = j)+Re+poly(Fr_logit,2)+Re*Fr_logit, data = trainData)
        fit.test = predict(spline, newdata=testData)
...
```{r}
spline <- glm(log(C_moment_3) ~ ns(St_log, df = 3)+Re+poly(Fr_logit,2)+Re*Fr_logit, data = trainData)
summary(spline)
```
...
formula <- c("log(C_moment_3) ~ log(St)+Re+logit(Fr)",
            "log(C_moment_3) ~ log(St)+Re+logit(Fr)+Re*logit(Fr)",
             ...
             "log(C_moment_3) ~ ns(log(St),df=3)+Re+poly(logit(Fr),2)+Re*logit(Fr)"
             )
df <- data.frame(models, formula, mse, R2)
df %>% 
  kbl() %>% 
