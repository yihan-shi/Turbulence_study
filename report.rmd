...
for(i in 1:K){
    ...
    #use k-fold cv to evaluate models
    for (j in 3:13){
        gam = gam(log(R_moment_3) ~ ns(St_log, df = j)+Re+ns(Fr_logit,2)+Re*Fr_logit, data = trainData)
...
```{r}
gam <- gam(log(R_moment_3) ~ ns(St_log, df = 3)+Re+ns(Fr_logit,2)+Re*Fr_logit, data = trainData)
summary(gam)
```
...
formula <- c("log(R_moment_3) ~ log(St)+Re+logit(Fr)",
            ...
            "log(R_moment_3) ~ ns(log(St),df=3)+Re+ns(logit(Fr),2)+Re*logit(Fr)"
             )
...
for(i in 1:K){
    ...
    #use k-fold cv to evaluate models
    for (j in 3:13){
        gam = gam(log(C_moment_3) ~ ns(St_log, df = j)+Re+ns(Fr_logit,2)+Re*Fr_logit, data = trainData)
        fit.test = predict(gam, newdata=testData)
...
```{r}
gam <- gam(log(C_moment_3) ~ ns(St_log, df = 3)+Re+ns(Fr_logit,2)+Re*Fr_logit, data = trainData)
summary(gam)
```
...
formula <- c("log(C_moment_3) ~ log(St)+Re+logit(Fr)",
            ...
            "log(C_moment_3) ~ ns(log(St),df=3)+Re+ns(logit(Fr),2)+Re*logit(Fr)"
             )
df <- data.frame(models, formula, mse, R2)
df %>% 
  kbl() %>% 
